 ...
 *
 * 统一两件事：
 * 1. `apiGet` —— 统一的响应处理与错误语义（404 单独标记，便于路由跳转）。
 * 2. `queryKeys` —— React Query 的键集中在同一处，避免各处手写字符串导致缓存不命中。
 *
 * 时序推进（超时 / 异步停止收尾 / 空闲释放 / 心跳回收）由云函数 poll 驱动
 * （docs/CLOUD-FUNCTION-WORKERS.md §2），前端不再打点懒处理。
 */
